...
for (hudson.model.Slave node in Jenkins.get().nodes.findAll { it instanceof hudson.slaves.DumbSlave }) {
  ...
  if (launcher instanceof CBLauncher) {
    ...
    println "Agent '${node.nodeName}' is already using the OSS SSH launcher. No action necessary."
  } else {
    // If launcher is using anything else, print status.
    println "Agent '${node.nodeName}' is not using a SSH launcher. No action necessary."
  }
}
